...
class GeminiEvaluator:
    ...
    def evaluate(self, structure: Dict[str, Any]) -> EvaluationResult:
        ...
        try:
            # プロンプトの取得とフォーマット
            prompt = self.controller.get_provider("gemini").get_template("structure_evaluation")
            if not prompt:
                raise PromptError("gemini", "structure_evaluation", "Template not found")
            
            formatted_prompt = prompt.format(structure=structure)
            
            # 評価の実行
            response = self.controller.generate_response("gemini", formatted_prompt)
            if not response:
            ...
            logger.error(f"Provider error in Gemini evaluation: {str(e)}")
            return EvaluationResult(
                is_valid=False,
                error=f"Provider error: {str(e)}"
            )
        except PromptError as e:
            logger.error(f"Prompt error in Gemini evaluation: {str(e)}")
            return EvaluationResult(
                is_valid=False,
                error=f"Prompt error: {str(e)}"
            )
        except Exception as e:
            logger.error(f"Unexpected error in Gemini evaluation: {str(e)}")
            return EvaluationResult(
                is_valid=False,

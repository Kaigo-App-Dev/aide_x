...
def test_register_and_get_template(mock_prompt_manager):
    """テンプレートの登録と取得のテスト"""
    # テンプレートの取得をテスト
    template = mock_prompt_manager.get("claude.test_template")
    assert template == "Hello {name}!"
    
    # 別のプロバイダーのテンプレートも取得可能
    template = mock_prompt_manager.get("chatgpt.test_template")
    assert template == "Hi {name}!"
    
    template = mock_prompt_manager.get("gemini.test_template")
...
def test_format_template(mock_prompt_manager):
    """テンプレートのフォーマットテスト"""
    # 正常なフォーマット
    formatted = mock_prompt_manager.format("claude.test_template", name="Alice")
    assert formatted == "Hello Alice!"
    
    # 別のプロバイダーのテンプレートもフォーマット可能
    formatted = mock_prompt_manager.format("chatgpt.test_template", name="Bob")
    assert formatted == "Hi Bob!"
    
    formatted = mock_prompt_manager.format("gemini.test_template", name="Charlie")
...
def test_format_template_missing_key(mock_prompt_manager):
    ...
    with pytest.raises(TemplateFormatError) as exc_info:
        mock_prompt_manager.format("claude.test_template")

...
def test_format_template(mock_prompt_manager):
    """テンプレートのフォーマットテスト"""
    # 正常なフォーマット
    formatted = mock_prompt_manager.format_template("claude", "test_template", placeholder="Alice")
    assert formatted == "Hello Alice!"
    
    # 別のプロバイダーのテンプレートもフォーマット可能
    formatted = mock_prompt_manager.format_template("chatgpt", "test_template", placeholder="Bob")
    assert formatted == "Hi Bob!"
    
    formatted = mock_prompt_manager.format_template("gemini", "test_template", placeholder="Charlie")
...
def test_format_template_missing_key(mock_prompt_manager):
    ...
    assert "Missing required key 'placeholder'" in str(exc_info.value)
...
def test_register_duplicate_template(mock_prompt_manager):
    """重複テンプレートの登録テスト"""
    # 同じテンプレートを再登録
    mock_prompt_manager.register_template("claude", "test_template", "Hello {placeholder}!")
    
    # 取得して確認
    template = mock_prompt_manager.get_template("claude", "test_template")
    assert template == "Hello {placeholder}!" 